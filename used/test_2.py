...
def predict(model, data, lookback=60, horizon=30):
    model.eval()
    x_input = data[['close', 'volume', 'ma20', 'rsi', 'divergence', 'd_range_pct']].values[-lookback:]
...
# 获取最近的10天数据用于预测
recent_data = df[df.index >= pd.Timestamp.now() - pd.Timedelta(days=10)]
predictions = []
actual_values = []

# 对每个时间点进行预测
for i in range(500 - lookback - horizon):
    test_data = recent_data.iloc[i:i + lookback]
    true_values = recent_data.iloc[i + lookback:i + lookback + horizon]['close'].values
