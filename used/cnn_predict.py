...
import torch.nn.functional as F  # 需要添加这行导入
...
class CNNModel(nn.Module):
    def __init__(self, input_size, num_features):
        ...
        self.adaptive_pool = nn.AdaptiveMaxPool1d(10)
...
model = CNNModel(input_size, num_features)
optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-5)
epochs = 200               # 可增加到200轮提高收敛性
batch_size = 128           # 根据GPU显存调整
criterion = nn.BCEWithLogitsLoss()  # 二分类交叉熵损失

# 保持训练函数和数据预处理流程不变
# ... 后续的train函数、评估代码与原始版本相同 ...
# 训练函数（修改后）
