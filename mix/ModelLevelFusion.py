...
class BaseCNN(nn.Module):
    ...
    def forward(self, x):
        return self.cnn(x.permute(0, 2, 1)).squeeze(-1)
...
class ModelLevelFusion(nn.Module):
    def __init__(self, cnn_model, rnn_model, lstm_model, weights=(0.3, 0.3, 0.4)):
        super().__init__()
    ...
    def forward(self, x_minute, x_30m):
        out_cnn = self.cnn_model(x_minute)
        out_rnn = self.rnn_model(x_minute)
        out_lstm = self.lstm_model(x_30m)
        out = self.weights[0] * out_cnn + self.weights[1] * out_rnn + self.weights[2] * out_lstm
        return out
...
# 初始化模型
cnn_model = BaseCNN(minute_input_size)
rnn_model = BaseRNN(minute_input_size, hidden_size)
lstm_model = BaseLSTM(thirty_min_input_size, hidden_size)

model = ModelLevelFusion(cnn_model, rnn_model, lstm_model, weights=(0.3, 0.3, 0.4))
