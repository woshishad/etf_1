...
class MultiScaleLSTM(nn.Module):
    def __init__(self, minute_input_size, thirty_min_input_size, hidden_size, output_size):
        super().__init__()
        # 分钟级LSTM
        self.minute_lstm = nn.LSTM(minute_input_size, hidden_size, batch_first=True)
        # 30分钟级LSTM
        self.thirty_min_lstm = nn.LSTM(thirty_min_input_size, hidden_size, batch_first=True)
        # 全连接层
    ...
    def forward(self, x_minute, x_30m):
        # 处理分钟级数据
        _, (h_minute, _) = self.minute_lstm(x_minute)
        # 处理30分钟级数据
        _, (h_30m, _) = self.thirty_min_lstm(x_30m)
        # 拼接特征
...
# 初始化模型
model = MultiScaleLSTM(minute_input_size, thirty_min_input_size, hidden_size, output_size)
